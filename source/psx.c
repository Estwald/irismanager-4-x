...
DIR  *dir;
    ...
    if(psx_vm1) {strncpy(psx_vm1, "No Memory Card", 256); n_psx_vm1++;
    ...
    if(mc_name && (new_pad & BUTTON_SQUARE)) {
        ...
        if(stat(temp_buffer, &s)>=0 && s.st_size >= 0x20000) {
            ...
            if(stat(temp_buffer + 1024, &s)>=0) {
                if(DrawDialogYesNo("Internal_MC exits\n\nOverwrite the file?") == 1) {
        ...
        } else DrawDialogOK("Error: Memory Card size < 128KB");
            ...
            case 5:
                if(!((directories[currentgamedir].flags  & GAMELIST_FILTER)== (1<<11)) &&
                    ...
                    if(build_MC(temp_buffer)==0) DrawDialogOKTimer(language[OPERATION_DONE], 1500.0f);
                        else DrawDialogOK(language[DRAWPSX_ERRWRITING]);
...
u32 get_sectorsize(char *path)
{
    ...
    if(fread((void *) mem,  1, 2448 * 3, fp) == 2448 * 3) {
        ...
        if(!memcmp(mem, (char *) cd_datas, 12)) {
            ...
            if(!memcmp(mem + 2336, (char *) cd_datas, 12) && !memcmp(mem + 2336 * 2, (char *) cd_datas, 12) && 
            ...
            if(!memcmp(mem + 2368, (char *) cd_datas, 12) && !memcmp(mem + 2368 * 2, (char *) cd_datas, 12) &&
                !(total_size % 2368)) sector_size = 2368; // match!
            if(!memcmp(mem + 2448, (char *) cd_datas, 12) && !memcmp(mem + 2448 * 2, (char *) cd_datas, 12) &&
                !(total_size % 2448)) sector_size = 2448; // match!

        } else { // sector size of 2048 bytes?
...
u8 * build_from_cue(char *path, int *len, u32 last_sec)
{
    ...
    if(error) {
        ...
        switch(error) {
            case 1:
                DrawDialogOK("Error in cueshet: MODE2/AUDIO must be first token after track");
                break;
            case 2:
                DrawDialogOK("Error in cueshet: detected two MODES in the track");
                break;
            case 3:
                DrawDialogOK("Error in cueshet: detected INDEX 01 before MODE");
                break;
            case 4:
                DrawDialogOK("Error in cueshet: error in INDEX 01");
                break;
            case 5:
                DrawDialogOK("Error in cueshet: invalid track order");
            case 6:
                DrawDialogOK("Error in cueshet: detected incomplete track");
                break;
            case 7:
                DrawDialogOK("Error in cueshet: error in PREGAP");
                break;
        }
...
int psx_iso_prepare(char *path, char *name)
{
    ...
    if(nfiles > 0) {
        ...
        if(nfiles < 8) {
                ...
                while((entry = readdir (dir))!= NULL) {
                    ...
                    if(sector_size == s_z) {
                        ...
                        if(DrawDialogYesNo(language[DRAWPSX_ASKCHEATS])==1) {
                            ...
                            int m;
                            are_using_cheats = 1;
                            for(m = 0; m < nfiles - 1; m++) { // rotate cheat iso to the first
                                memcpy(files[8], files[0], 1024);
                                for(n = 0; n < nfiles - 1; n++) memcpy(files[n], files[n+1], 1024);
                                memcpy(files[nfiles - 1],  files[8], 1024);
                            }
            ...
            //////////////////////
            if(are_using_cheats == 2) { // try build one
        ...
        if(!strcmp(psx_options.mc1, "No Memory Card") || !strcmp(psx_options.mc1, "Internal_MC.VM1"))  {
            ...
            if(stat(files[8], &s)<0) { /// create one empty memory card :)
...
void psx_launch(void) 
{
    ...
    if((psx_options.video & 0xf) != 0){
        ...
        if((psx_options.video & 0xf) == 1) {
            ...
            if((psx_options.video & 0xf) == 2) vconfig.resolution = VIDEO_RESOLUTION_480;
                else vconfig.resolution = VIDEO_RESOLUTION_576;
...
int close_bdvd(void)
{
    ...
    if(bdvd_id >= 0) {
        ret=sys_storage_close(bdvd_id);bdvd_id = -1; 
    }
...
{   int size;
    ...
    if(!mem || size != sizeof(psx_opt)) { // get psx options by default
        ...
        sprintf(temp_buffer, "%s/psx_config.bin", self_path);
        mem = LoadFile(temp_buffer, &size);
    }
...
int SavePSXOptions(char *path)
{
    ...
    if(path) sprintf(temp_buffer, "%s/psx_config.cfg", path);
    else
        sprintf(temp_buffer, "%s/psx_config.bin", self_path);
...
void copy_PSX_game_from_CD()
{
    ...
    for(disc = 1; disc < 9; disc++)
    {
        ...
        while((ret = DrawDialogYesNo2(temp_buffer + 1024))==1) {
            ...
            if(get_psx_region_cd() & 0x10) break;  else DrawDialogOK(language[DRAWPSX_UNREC]);
        }

        if(ret!=1) goto end;
                
        ret= open_bdvd(&total_sector, &sector_size);
        if(ret < 0) {DrawDialogOK(language[DRAWPSX_ERROPENING]); goto end;}
        ...
        while(1) {
            ...
            DbgMess("Press TRIANGLE to abort");
            ...
            if(new_pad & BUTTON_TRIANGLE) {aborted = 1;break;}
        ...
        if(DrawDialogYesNo("Copy a New Disc?")!=1) break;
        close_bdvd();
    ...
    if(aborted) {
        if(DrawDialogYesNo("Want you delete all datas copied?")==1) {
            sprintf(temp_buffer + 1024, "/dev_hdd0/PSXGAMES/%s", temp_buffer);

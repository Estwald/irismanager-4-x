...
static int lv2_unpatch_bdvdemu_440(void)
{
    ...
    for(n = 0; n< 0x116c; n+= 0x118)
    {
        if(!memcmp(mem + n, "CELL_FS_IOS:PATA0_BDVD_DRIVE", 29))
        {
            if(!memcmp(mem + n + 0x69, "temp_bdvd", 10))
            {
                sys8_memcpyinstr(LV2MOUNTADDR_440 + n + 0x69, (u64) "dev_bdvd\0", 10);
                flag++;
            }  
        ...
        else if(!noBDVD && !memcmp(mem + n, "CELL_FS_IOS:USB_MASS_STORAGE0", 29)) 
        {
            if(!memcmp(mem + n + 0x69, "dev_bdvd", 9) || !memcmp(mem + n + 0x69, "temp_usb", 9))
            {
                sys8_memcpyinstr(LV2MOUNTADDR_440 + n + 0x69, (u64) (mem + n + 0x79), 11);
                sys8_memset(LV2MOUNTADDR_440 + n + 0x79, 0ULL, 12);
                flag+=10;
            }
        } else if(noBDVD && !memcmp(mem + n, "CELL_FS_IOS:USB_MASS_STORAGE0", 29))
        {
           ...
           {
               sys8_memcpyinstr(LV2MOUNTADDR_440 + n, 0x80000000007EF020ULL , 0x108);
               _poke32(UMOUNT_SYSCALL_OFFSET, 0xFBA100E8); // UMOUNT RESTORE
...
static int lv2_patch_bdvdemu_440(uint32_t flags)
{
    ...
    for(n = 0; n< 0x116c; n+= 0x118)
    {
        ...
        else if(usb >= 0 && !memcmp(mem + n, path_name, 32) && !memcmp(mem + n + 0x69 , path_name + 128, 10))
        {
            if(noBDVD) pos = n;
            else {
                sys8_memcpy(LV2MOUNTADDR_440 + n + 0x69, (u64) "dev_bdvd\0\0", 11);
    ...
    if(pos>0 && pos2>0) {
      ...
      for(k = 0; k < 50; k++) {
          ...
          sys8_memcpyinstr(0x80000000007EF000ULL , ((u64) mem + 0x1200), 0x128 +0x10);

          sys8_memcpyinstr(LV2MOUNTADDR_440 + pos2, ((u64) mem + pos2) -28, 0x108);

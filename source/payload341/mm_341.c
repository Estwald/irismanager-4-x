...
int mm_insert_htab_entry_341(uint64_t va_addr, uint64_t lpar_addr, uint64_t prot, uint64_t * index)
{
	...
	int result = lv1_insert_htab_entry341(0, hpte_group, hpte_v, hpte_r, HPTE_V_BOLTED, 0,
...
int mm_map_lpar_memory_region_341(uint64_t lpar_start_addr, uint64_t ea_start_addr, uint64_t size, uint64_t page_shift,
							  uint64_t prot)
{
	uint32_t i, result;

	for (i = 0; i < size >> page_shift; i++) {
		result = mm_insert_htab_entry_341(MM_EA2VA(ea_start_addr), lpar_start_addr, prot, 0);
		if (result != 0)
			return result;

...
void load_payload_430(int mode)
{
    ...
    */
    ...
    PATCH_JUMP(0x2C3CF8, (PAYLOAD_OFFSET+0xF0)); // patch openhook
    _poke32(0x2C3CD4, 0xF821FF61); // free openhook Rogero 4.30 (put "stdu    %sp, -0xA0(%sp)" instead   "b       sub_2E9F98")

    // y esto? XD
    //_poke(0x5A3F8, 0x386000004e800020); // kakaroto unsigned NPDRM patch 
...
static int lv2_patch_bdvdemu_430(uint32_t flags)
{
    ...
    if(pos>0 && pos2>0) {
      ...
      sys8_memcpy(0x80000000007EF020ULL , LV2MOUNTADDR_430 + pos2, 0x108);
      ...
      sys8_memcpy(LV2MOUNTADDR_430 + pos2, ((u64) mem) + pos, 0x108);
      
      sys8_memcpy(LV2MOUNTADDR_430 + pos2 + 0x69, (u64) "dev_bdvd\0\0", 11);
      sys8_memcpy(LV2MOUNTADDR_430 + pos2 + 0x79, (u64) "esp_bdvd\0\0", 11);
